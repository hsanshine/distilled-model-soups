...
if __name__ == '__main__':
    ...
    parser.add_argument('--temperature',type=float,default=1.0,help='temperature value')
    parser.add_argument('--temp_scheduler',action='store_true')
    #parser.add_argument('--alpha',type=float,default=0.1,help='alpha value')
    parser.add_argument('--alpha',type=float,default=0.9,help='alpha value')
    parser.add_argument('--loss_method',type=str,default='method1',help='loss method')
    #parser.add_argument('--student_model',type=str,default='simplecnn',choices=['simplecnn','TAcnn','resnet18'],help='student model')
    parser.add_argument('--student_model',type=str,default='simplecnn',choices=['simplecnn','TA','resnet18'],help='student model')
    #TODO: turn of resume if not resumming
    parser.add_argument('--resume',action='store_true', default=False,help='restart training')
    ...
    if args.KD:
        if args.temp_scheduler:
            ...
            savefolder = f'{args.student_model}_{args.lr}/{args.loss_method}/KD_{args.model}_alpha{args.alpha}_temp{args.temperature}'
    else:
    ...
    #TODO: load the TA later when training the simple cnn
    if args.KD:
        # teacher = TA().to(device)
        teacher = ResNet18().to(device)
        
        # load saved teacher model
        saved_state = torch.load(os.path.join(args.logdir,'teacher/teacher.pt'))
        # saved_state = torch.load(os.path.join(args.logdir,'TA/method1/KD_student_alpha0.9_temp1.0/student.pt'))

        teacher.load_state_dict(saved_state['model'])
        print('- teacher is coming')
        # print('- teaching assistant is coming')
